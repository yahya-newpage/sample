...
?>
	...
	<div>
		...
		<tr>
			<td>
			<table  border=0 width=100% cellpadding=2 cellspacing =2 class="maincontents1_center_changelog_view">
				<tr>
				<td>author  </td><td><a href='./sendmail.php?mail=<?=$mail?>&commit=<?=$commit?>' class='mail2author'><?=$author?></a></td>
				</tr>
				<tr>
				<td> </td><td> <?=$authordate_utc?>&nbsp;&nbsp;(KST : <?=$authordate_kst?>)</a></td>
				</tr>
				<tr>
				<td>commit      </td><td><?=$commit?></td>
				</tr>
				<tr>
				<td>  </td><td><?=$commitdate_utc?>&nbsp;&nbsp;(KST : <?=$commitdate_kst?>)</a></td>
				</tr>
		...
		<tr height=620px>
			<td valign=TOP>
				<div id='commit_view_contents' style="font-size:12px; font-family: Courier New;">
				...
				<div id='commit_view_files'>					
					<?
						$filenum = 0;
						$sub=1;
						$handle=popen($epath."git $path  diff-tree $commit -r","r");
						while(!feof($handle)){
							$buffer = fgets($handle);
							if(preg_match(
				...
				<div id="commit_view_contents2">
					<?
						$sub=0;
						include_once($GSTAT[GESHIDIR].'/geshi.php');
						$handle=popen($epath."git $path diff-tree $commit --patch-with-raw","r");
						while(!feof($handle)) {
							$buffer = fgets($handle);
